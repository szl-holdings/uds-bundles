...
> **a11oy vertical re-issue — a11oy Sentinel.** This signed deployment bundle is the
> a11oy-vertical re-bundling of the `sentra` organ. The bundle, Zarf package,
> namespace and UDS Package CR are re-issued and re-signed (keyless cosign OIDC)
> under the a11oy-vertical name `szl-a11oy-sentinel`. The underlying container image
> retains its internal codename (`ghcr.io/szl-holdings/sentra`) and continues
> to serve its `/api/sentra/` paths — its build source is archived and is not
...
- Deploys in 60 seconds on UDS Core via `zarf package deploy szl-a11oy-sentinel.uds.tar.zst --confirm`
...
| SLSA provenance (L1+L2, cosign-verified) | `attestations/sentra.slsa-provenance.json` |
...
zarf package deploy szl-a11oy-sentinel.uds.tar.zst --confirm

# 3. Watch it come up
kubectl -n szl-a11oy-sentinel get pods -w
```
